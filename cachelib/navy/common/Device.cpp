...
class FileDevice : public Device {
 ...
 private:
  ...
  // Enable async IO (io_uring or libaio) with sync or async completions
  const bool enableAsyncIO_;
  ...
  // Use io_uring instead libaio for async IO
...
FileDevice::FileDevice(std::vector<folly::File>&& fvec,
    ...
    : Device(fileSize * fvec.size(),
      ...
      enableAsyncIO_(qDepthPerContext >= 1),
  ...
  XLOGF(INFO,
        ...
        !enableAsyncIO_ ? "sync" : (useIoUring_ ? "io_uring" : "libaio"),
        qDepthPerContext_);
}
