...
FdpNvme::FdpNvme(const std::string& blockDevice)
    : file_(openNvmeCharFile(blockDevice)) {
  initializeFDP(blockDevice);
  XLOGF(INFO, "Initialized NVMe FDP Device on file: {}", blockDevice);
}
...
void FdpNvme::initializeFDP(const std::string& blockDevice) {
  nvmeData_ = readNvmeInfo(blockDevice);
...
// It returns "nvme0n1" for both "/dev/nvme0n1" and "/dev/nvme0n1p1"
std::string getNvmeBlockName(const std::string& blockDevice) {
  size_t bPos = blockDevice.find_last_of("/") + 1;
  size_t pPos = blockDevice.find('p', bPos);

  XDCHECK(bPos < std::string::npos);
  return blockDevice.substr(bPos, pPos - bPos);
}

// It returns nvme0n1p1 for /dev/nvme0n1p1 (partitioned NS),
// and returns "" for /dev/nvme0n1 (non-partitioned NS)
std::string getNvmePartName(const std::string& blockDevice) {
  size_t bPos = blockDevice.find_last_of("/") + 1;
  size_t pPos = blockDevice.find('p', bPos);

  XDCHECK(bPos < std::string::npos);
  if (pPos == std::string::npos) {
    return {}; // The block device is just an NS; not a partition
  }
  return blockDevice.substr(bPos, std::string::npos - bPos);
}

// Read the /sys/block/xx entry for a block device
...
// Get the Namespace ID of a NVMe block device
int getNvmeNsId(const std::string& bName) {
  return (folly::to<int>(readDevAttr(bName, "nsid")));
}

// Get the Max Transfer size in bytes for an NVMe block device
uint32_t getMaxTfrSize(const std::string& bName) {
  // max_hw_sectors_kb : This is the maximum number of kilobytes supported in a
  // single data transfer.
  // (https://www.kernel.org/doc/Documentation/block/queue-sysfs.txt)
  return (1024u * /* multiply by kb */
          folly::to<uint32_t>(readDevAttr(bName, "queue/max_hw_sectors_kb")));
}

// Get LBA shift of an NVMe block device
uint32_t getLbaShift(const std::string& bName) {
  return folly::constexpr_log2(
      folly::to<uint32_t>(readDevAttr(bName, "queue/logical_block_size")));
}

// Get the partition start in bytes
uint64_t getPartStart(const std::string& bName, const std::string& pName) {
  return (512u * /* sysfs size is in terms of linux sector size */
          folly::to<uint64_t>(readDevAttr(bName + "/" + pName, "start")));
}

NvmeData FdpNvme::readNvmeInfo(const std::string& blockDevice) {
  auto bName = getNvmeBlockName(blockDevice);
  auto pName = getNvmePartName(blockDevice);

  try {
    int namespace_id = getNvmeNsId(bName);
    uint32_t lbaShift = getLbaShift(bName);
    uint32_t maxTfrSize = getMaxTfrSize(bName);

    uint64_t startLba{0};
    if (!pName.empty()) {
      startLba = getPartStart(bName, pName) >> lbaShift;
    }
    ...
    XLOGF(ERR, "Exception in openNvmeCharFile for: {}", blockDevice);
    throw;
  }
}
...
std::string getNvmeCharDevice(const std::string& blockDevice) {
...
folly::File FdpNvme::openNvmeCharFile(const std::string& fileName) {
  if (!isValidNvmeDevice(fileName)) {
  ...
  try {
    auto charFileName = getNvmeCharDevice(fileName);
    XLOGF(INFO, "Opening NVMe Char Dev file: {}", charFileName);
    f = folly::File(charFileName.c_str(), flags);
  } catch (const std::system_error& e) {
    XLOGF(ERR, "Exception in openNvmeCharFile for: {}", fileName);
    throw;
  }
  XDCHECK_GE(f.fd(), 0);
